            ...
            dataitem(Integer; Integer)
            {
                ...
                begin
                    ...
                    if ReservationEntry.FindSet() then
                        SetRange(Number, 1, ReservationEntry.Count)
                    else
                        SetRange(Number, 1, 1);
                end;
                ...
                begin
                                ...
                                if ItemLedgEntry.FindFirst() then begin
                                    if ItemLedgEntry."Entry Type" = ItemLedgEntry."Entry Type"::Purchase then begin
                                        ...
                                        if ItemLedgEntry."Entry Type" = ItemLedgEntry."Entry Type"::Transfer then begin
                                            if ItemLedgEntry."Document Type" = ItemLedgEntry."Document Type"::"Transfer Receipt" then
                                                QtyDelivered := ItemLedgEntry.Quantity;
                                        end;
                                end;
                            end;
                    end;
                    ...
                    if ((ETA > "sales line"."Planned Delivery Date") and (ETA <> 0D)) then begin
                        repeat
                            if NextWorkingday = 0D then
                                NextWorkingday := "sales line"."Planned Delivery Date"
                            else
                                NextWorkingday := NextWorkingday + 1;

                            if not CalendarMgt.IsNonworkingDay(NextWorkingday, CustomCalendarChange) then
                                DayDiff += 1;
